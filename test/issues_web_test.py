...
@jira_issues("cheshimantu/allure-pytest#3")
@pytest.mark.web
...
@tm4j("AE-T4")
@microservice("Repository")
@allure.story("Create new note to ads")
@jira_issues("cheshimantu/allure-pytest#3")
@pytest.mark.web
@pytest.mark.regress
@allure.title("Adding note to advertisement")
def test_should_add_ads_note(web_driver):
    steps.open_issues_page(OWNER, REPO)
    steps.create_note_with_title(ISSUE_TITLE)
    steps.should_see_note_with_title(ISSUE_TITLE)
